...
async function loadCurrent(event: any): Promise<Record<string, any>> {
  ...
  if (!where) return {}
  const existing = await strapi.db.query('api::achievement.achievement').findOne({ where, populate: ['project'] })
  return existing ?? {}
}
...
async function checkSlugUnique(event: any, merged: Record<string, any>) {
  ...
  const count = await strapi.db.query('api::achievement.achievement').count({ where })
  if (count > 0) {
